...
export const VIRTUAL_MODULE_ID = 'virtual:discogs-collection/images';
const RESOLVED_VIRTUAL_MODULE_ID = '\0' + VIRTUAL_MODULE_ID;

export function discogsCollectionImagesPlugin(): Plugin {
  return {
    name: 'astro-discogs-collection:images',
    resolveId(id) {
    ...
    load(id) {
      ...
      files.forEach((file, index) => {
        ...
        if (!releaseId) return; // not a file this cache manages (e.g. leftover from before a naming change)
        ...
        entries.push(`  ${JSON.stringify(releaseId)}: ${varName}`);
      });
