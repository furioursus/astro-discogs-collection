...
import { configure, type DiscogsCollectionOptions } from './config.js';
    ...
    hooks: {
      'astro:config:setup': ({ config, updateConfig, logger }) => {
        ...
        if (resolved.missingConfig) {
          logger.warn(
            'DISCOGS_USERNAME/DISCOGS_TOKEN are not set (via integration options or environment variables) — ' +
